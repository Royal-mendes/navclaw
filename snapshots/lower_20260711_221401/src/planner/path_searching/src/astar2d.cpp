...
{
  int effective_safety_mode = safety_mode;
  double effective_max_time = max_time;
  // Frontier exploration calls use this exact parameter pair. Override here so
  // other A* callers keep NORMAL mode and their original time budget.
  if (safety_mode == SAFETY_MODE::NORMAL && std::abs(success_dist - 0.25) < 1e-6 &&
      std::abs(max_time - 0.01) < 1e-6) {
    effective_safety_mode = SAFETY_MODE::OPTIMISTIC;
    effective_max_time = 2.0;
  }
  ...
  while (!open_set_.empty()) {
    ...
    if ((ros::Time::now() - t1).toSec() > effective_max_time) {
    ...
    for (auto step : steps) {
      ...
      if ((nbr_pos - start_pt).norm() > 0.25) {
        // Check safety
        if (!checkPointSafety(nbr_pos, effective_safety_mode))
          continue;
        ...
        for (double l = 0.025; l < len; l += 0.025) {
          ...
          if (!checkPointSafety(ckpt, effective_safety_mode)) {
            safe = false;
            break;
          }
        }
