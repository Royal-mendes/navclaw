...
void ObjectMap2D::getObjectSummaries(vector<ObjectClusterSummary>& summaries) const
{
  summaries.clear();
  for (const auto& object : objects_) {
    ObjectClusterSummary summary;
    summary.id = object.id_;
    summary.best_label = object.best_label_;
    summary.cell_count = static_cast<int>(object.cells_.size());
    summary.good_cell_count = static_cast<int>(object.good_cells_.size());
    summary.center = object.average_;
    summary.box_min2d = object.box_min2d_;
    summary.box_max2d = object.box_max2d_;
    summary.box_min3d = object.box_min3d_;
    summary.box_max3d = object.box_max3d_;

    if (object.best_label_ >= 0 &&
        object.best_label_ < static_cast<int>(object.confidence_scores_.size())) {
      summary.best_score = object.confidence_scores_[object.best_label_];
      summary.best_observation_count = object.observation_nums_[object.best_label_];
    }
    if (!object.confidence_scores_.empty()) {
      summary.target_score = object.confidence_scores_[0];
      summary.target_observation_count = object.observation_nums_[0];
    }
