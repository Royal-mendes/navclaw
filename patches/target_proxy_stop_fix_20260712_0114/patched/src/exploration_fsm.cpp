  ...
  switch (state_) {
    ...
    case ROS_STATE::PLAN_ACTION: {
      ...
      if (fd_->init_action_count_ < 1 + 12 + 1 + 12) {
        const int init_action_index = fd_->init_action_count_;
        if (init_action_index < 1)
          fd_->newest_action_ = ACTION::TURN_DOWN;
        else if (init_action_index < 1 + 12)
          fd_->newest_action_ = ACTION::TURN_LEFT;
        else if (init_action_index < 1 + 12 + 1)
          fd_->newest_action_ = ACTION::TURN_UP;
        else
          fd_->newest_action_ = ACTION::TURN_LEFT;
        ROS_WARN("Init Mode Process -----> (%d/26)", init_action_index);
        fd_->init_action_count_++;
        transitState(ROS_STATE::PUB_ACTION, "FSM");
        updateFrontierAndObject();
        const int horizontal_scan_start = 1 + 12 + 1;
        const int horizontal_scan_end = horizontal_scan_start + 12;
        if (init_action_index >= horizontal_scan_start &&
            init_action_index < horizontal_scan_end &&
            ((init_action_index - horizontal_scan_start) %
                expl_manager_->ep_->vlm_initial_panorama_collect_every_n_init_turns_ == 0)) {
          expl_manager_->recordInitialVLMPanoramaView(
              Eigen::Vector2d(fd_->odom_pos_(0), fd_->odom_pos_(1)), fd_->odom_yaw_,
              expl_manager_->ed_->frontier_averages_, init_action_index);
        }
      }
      else {
...
int ExplorationFSM::callActionPlanner()
{
  ...
  if (expl_manager_->consumeReachedVLMTargetObjectProxyStop()) {
    ROS_ERROR("[VLM Target] Reached model-selected target_object_proxy with current detector "
              "evidence; stopping episode without requiring a duplicate proxy selection.");
    final_res = FINAL_RESULT::REACH_OBJECT;
    return final_res;
  }

  if (expl_manager_->shouldStopOracleRolloutAtTarget(current_pos)) {
    ROS_ERROR("Frontier oracle reached the private GT target radius; stopping episode.");
    final_res = FINAL_RESULT::REACH_OBJECT;
    return final_res;
  }
  ...
  int forced_vlm_action = -1;
  if (expl_manager_->consumePendingVLMForcedAction(forced_vlm_action)) {
    fd_->newest_action_ = forced_vlm_action;
  ...
  if (fd_->dormant_frontier_flag_) {
    expl_manager_->clearActiveVLMWaypoint("frontier_dormant_or_failed");
    fd_->replan_flag_ = true;
    fd_->dormant_frontier_flag_ = false;
  }
  else if (expl_manager_->hasActiveVLMWaypoint()) {
    // Keep the currently selected VLM waypoint authoritative until it is reached or cleared.
    fd_->replan_flag_ = true;
  }
  ...
  if (final_res == FINAL_RESULT::EXPLORE) {
    // Force dormant if a normal frontier is very close but still exploring. VLM-selected
    // active waypoints use their own tighter reached threshold so local relay points are
    // followed to the selected goal instead of being cleared at the frontier dormant radius.
    if (!expl_manager_->hasActiveVLMWaypoint() &&
        min_dist < FSMConstants::FORCE_DORMANT_DISTANCE) {
  ...
  if (fd_->stucking_action_count_ >= FSMConstants::MAX_STUCKING_COUNT) {
      ...
      expl_manager_->clearActiveVLMWaypoint("stuck_for_too_long");
