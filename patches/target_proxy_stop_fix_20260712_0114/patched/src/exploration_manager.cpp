...
bool ExplorationManager::consumeReachedVLMTargetObjectProxyStop()
{
  ...
  // the controller reached it. Require current detector-map evidence as the final
  // confirmation, but do not require the now-reached proxy to be generated a second time.
  if (target_point_count == 0) {
    vlm_target_object_proxy_ready_to_stop_ = false;
    ROS_WARN("[VLM Target] Reached target_object_proxy but current target evidence is empty; "
             "clear stop latch and require a new model-confirmed target proxy.");
    return false;
  }

  vlm_target_object_proxy_ready_to_stop_ = false;
  ROS_WARN("[VLM Target] Consume reached target_object_proxy stop latch with current "
           "target evidence: clouds=%zu, points=%zu",
