...
The first implementation deliberately has a narrow contract:
...
- API and parsing failures never create a geometric or nearest-candidate
  fallback;
- every observation, raw model reply, validation result, and selected action is
  logged.
...
- `bridge/selector_client.py`: CLI-compatible replacement for the current Python selector.
